...
object <- readRDS("~/atvTrem2_combined_objects/NGS000072_integrated.Rds")
...
write.csv(markers, "~/atvTrem2_combined_scRNAseq/inst/extdata/NGS000072_cluster_markers.csv")
```

```{r}
markers <- read.csv("~/atvTrem2_combined_scRNAseq/inst/extdata/NGS000072_cluster_markers.csv", row.names = 1, header = T)

top10 <- markers %>% group_by(cluster) %>% slice_max(n = 10, order_by = avg_log2FC)
```

```{r, fig.width=6, fig.height=3}
FeaturePlot(object, c("P2ry12", "Slc1a2", "F13a1", "Cldn5", "S100a11", "Atp1a2"), ncol = 3)
```
...
saveRDS(microglia, file = "~/atvTrem2_combined_objects/NGS000072_microglia.Rds")
```
