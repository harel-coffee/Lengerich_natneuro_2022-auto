...
```{r, include = FALSE}
Packages <- c("atvtrem2Combined", "dplyr", "Seurat", "aws.s3", "Matrix", "ggplot2")
...
```{r, include=FALSE}
integrated_object <- aws.s3::s3readRDS(bucket = bucket,
                                       object = "ATV-Trem2_APPKI_all_samples_integrated.Rds",
                                       region = "us-west-2")
```
...
write.csv(markers, file = "~/DNLI-21-236_ATV-Trem2_APPKI_timecourse/inst/extdata/cluster_markers.csv")
```

```{r}
DefaultAssay(integrated_object) <- "RNA"
markers <- read.csv("~/DNLI-21-236_ATV-Trem2_APPKI_timecourse/inst/extdata/cluster_markers.csv", header = T, row.names = 1)
top10 <- markers %>% group_by(cluster) %>% top_n(wt = avg_log2FC, n = 10)
```

```{r, fig.width=12, fig.height=5}
FeaturePlot(integrated_object, features = c("F13a1", "Ccr2", "Cd3g","Retnlg", "Sparcl1", "Mog", "nFeature_RNA", "P2ry12"), ncol = 4)
```


```{r, include=FALSE, eval=FALSE}
microglia <- subset(integrated_object, subset = seurat_clusters %in% c(0,1,2,3,4,5))
```
...
saveRDS(microglia_integrated, file ="~/NGS000123_objects/ATV-Trem2_APPKI_microglia_integrated.Rds")
