...
set.seed(1234)
bucket1 <- "com.dnli.bioinfo.data/processed/internal/Study2/remapped"
...
```{r, include=FALSE}
microglia_Study1 <- aws.s3::s3readRDS(bucket = bucket2,
                                     object = "Study1_microglia_subset.Rds",
                                     region = "us-west-2")
  
microglia_Study2 <- aws.s3::s3readRDS(bucket = bucket1,
...
DimPlot(microglia_Study1, label=T) + DimPlot(microglia_Study2, group.by = "seurat_clusters",label = T)
```

And here they are split by treatment:
```{r, fig.width=6, fig.height=3}
DimPlot(microglia_Study1, split.by = "group", ncol = 3) + NoAxes()
```

```{r, fig.width=5, fig.height=3}
DimPlot(microglia_Study2, split.by = "group", group.by = "seurat_clusters", ncol = 3) + NoAxes()
```
...
anchors73 <- Seurat::FindTransferAnchors(reference = map, query = microglia_Study1)
predictions_Study1 <- TransferData(anchorset = anchors73, refdata = map$integrated_microglia_state)
microglia_Study1 <- AddMetaData(microglia_Study1, predictions_Study1)
rm(anchors73)

anchors123 <- Seurat::FindTransferAnchors(reference = map, query = microglia_Study2)
predictions_Study2 <- TransferData(anchorset = anchors123, refdata = map$integrated_microglia_state)
microglia_Study2 <- AddMetaData(microglia_Study2, predictions_Study2)
rm(anchors123)

future::plan("default")

microglia_Study1$predicted.id <- factor(microglia_Study1$predicted.id, 
                                                    c("Homeostatic",
                                                      "DAM/Homeostatic",
                                                      "DAM_Indistinct",
                                                      "DAM_Canonical",
...
microglia_Study2$predicted.id <- factor(microglia_Study2$predicted.id, 
                                                    c("Homeostatic",
                                                      "DAM/Homeostatic",
                                                      "DAM_Indistinct",
                                                      "DAM_Canonical",
...
DimPlot(microglia_Study1, group.by = "predicted.id",cols = state_colors) + 
  DimPlot(microglia_Study2, group.by = "predicted.id",cols = state_colors)
```

```{r, include=FALSE}
totals_73 <- as.data.frame((table(microglia_Study1$predicted.id, microglia_Study1$sample_id))) %>%
  group_by(Var2) %>%
...
```{r, include=FALSE}
totals_123 <- as.data.frame((table(microglia_Study2$predicted.id, microglia_Study2$sample))) %>%
  group_by(Var2) %>%
  mutate(percent = Freq/sum(Freq) *100)
...
FeaturePlot(microglia_Study1, features = chen_score_names, min.cutoff = 0, order = T, ncol = 3) &
  scale_color_gradientn(colors = brewer.pal("Oranges", n = 9)) &
  NoAxes()
```

```{r, echo=FALSE, fig.width=12, fig.height=6, message=FALSE}
FeaturePlot(microglia_Study2, features = chen_score_names, min.cutoff = 0, order = T, ncol = 3) &
  scale_color_gradientn(colors = brewer.pal("Oranges", n = 9)) &
  NoAxes()
```
...
FeaturePlot(microglia_Study1, features = topics_score_names, min.cutoff = 0, order = T, ncol = 3) &
  scale_color_gradientn(colors = brewer.pal("Oranges", n = 9)) &
  NoAxes()
```

```{r, echo=FALSE, fig.width=12, fig.height=9, message=FALSE}
FeaturePlot(microglia_Study2, features = topics_score_names, min.cutoff = 0, order = T, ncol = 3) &
  scale_color_gradientn(colors = brewer.pal("Oranges", n = 9)) &
  NoAxes()
```
