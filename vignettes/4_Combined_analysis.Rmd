...
DimPlot(microglia_NGS73, group.by = "group", shuffle = T, raster = T, raster.dpi = c(1024,1024)) + DimPlot(microglia_NGS123, group.by = "group",label = F, shuffle = T, raster = T, raster.dpi = c(1024,1024)) & NoAxes()
```

```{r}
table(microglia_NGS123$group, microglia_NGS123$sample)
```

```{r}
table(microglia_NGS73$group, microglia_NGS73$sample_id)
```
...
```{r}
microglia_NGS73 <- subset(microglia_NGS73, subset = sample_id != "NH36G")
```
...
```{r}
microglia_NGS123$sample_id <- microglia_NGS123$sample
microglia_NGS123$barcode <- rownames(microglia_NGS123@meta.data)
microglia_NGS123$study <- "Study2"
microglia_NGS73$study <- "Study1"



cells_to_keep1 <- microglia_NGS73@meta.data %>% group_by(sample_id) %>% slice_sample(n = 1400)
cells_to_keep2 <- microglia_NGS123@meta.data %>% group_by(sample_id) %>% slice_sample(n = 1400)

microglia_NGS73 <- subset(microglia_NGS73, cells = cells_to_keep1$barcode)
microglia_NGS123 <- subset(microglia_NGS123, cells = cells_to_keep2$barcode)

```

We will combine these studies using integration, to remove any study-specific effects.
```{r}
microglia_NGS73 <- NormalizeData(microglia_NGS73)
microglia_NGS73 <- FindVariableFeatures(microglia_NGS73)

microglia_NGS123 <- NormalizeData(microglia_NGS123)
microglia_NGS123 <- FindVariableFeatures(microglia_NGS123)
```

```{r, eval = FALSE}
anchors <- FindIntegrationAnchors(list(microglia_NGS73, microglia_NGS123), anchor.features = 2000)
...
DimPlot(combined, split.by = "study", group.by = "clusters",cols = clustercols, raster = T, raster.dpi = c(1024,1024)) & NoAxes()
```
...
```{r, include=FALSE}
totals <- as.data.frame((table(combined$clusters, combined$sample_id))) %>%
  group_by(Var2) %>%
  mutate(percent = Freq/sum(Freq) *100)

colnames(totals) <- c("cluster", "sample_id", "Count", "Percent")

meta_slim <- combined@meta.data %>% select(c(group, sample_id, study)) %>% distinct(.)

totals <- totals %>%
  left_join(meta_slim, by = "sample_id")

totals$cluster <- factor(totals$cluster, levels = rev(levels(totals$cluster)))
```
...
```{r}
totals$group <- factor(totals$group, levels = rev(levels(totals$group)))
...
```{r, fig.width=10, fig.height=3.5}
DefaultAssay(combined) <- "RNA"
FeaturePlot(combined, c("Tmem119", "Cst7", "Apoe","Lpl", "Pcna", "Mki67", "Cxcl10", "Aldoa"), ncol = 4, order = F) & viridis::scale_color_viridis() & NoAxes()
```
...
FeaturePlot(combined, paste0("ChenColonna_", names(chen_sigs), "1"), ncol = 5, order = F, min.cutoff = 0) & viridis::scale_color_viridis() & NoAxes()
```


```{r}
sample_annotation <-  combined@meta.data %>% select(c(group, sample_id)) %>% distinct(.)
...
```{r}
chen_averages <- AverageExpression(combined, features = rownames(chen_annotation), group.by = "sample_id")

chen_cluster_averages <- AverageExpression(combined, features = rownames(chen_annotation), group.by = "seurat_clusters")
```
...
pheatmap::pheatmap(mat = chen_cluster_averages$RNA, 
                   scale = "row",
                   annotation_row = chen_annotation,
