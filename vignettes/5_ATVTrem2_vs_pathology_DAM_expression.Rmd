...
```{r, include = FALSE}
Packages <- c("atvtrem2Combined", "dplyr", "Seurat", "aws.s3", "Matrix", "ggplot2", "pheatmap", "RColorBrewer", "scuttle", "SingleCellExperiment", "limma", "msigdbr", "fgsea", "sparrow", "edgeR")
...
We're going to look at how DAM-like microglia are induced by both ATV:Trem2 treatments and real AD-like pathology (Abeta plaques). We'll take both the NGS000123 data set (APP KI mice treated with ATV:Trem2) and the NGS000073 data set (the original ATV-Trem2 treated mouse scRNA-seq. All WT). 

We'll do some expression comparisons at the pseudobulk level and also combine everything together to see how the populations cluster.

The main goal of this analysis is to answer the question: **How similar are ATV:Trem2-activated microglia to patholgy-activated microglia?**

```{r, include=FALSE}
microglia_NGS73 <- aws.s3::s3readRDS(bucket = bucket2,
                                     object = "NGS000073_final_seurat_object.RDS",
                                     region = "us-west-2")
...
microglia_NGS123$group <- plyr::mapvalues(microglia_NGS123$group, from = c('WT_ATV-DNP02',      
                                                                         'WT_ATV-4D9',        
                                                                         'APP-SAA_ATV-DNP02', 
                                                                         'APP-SAA_ATV-4D9',   
                                                                         'APP-SAA 4D9'),
                                                                  to = c("NGS123_WT_ATV_DNP02",
                                                                         "NGS123_WT_ATV_4D9",
                                                                         "NGS123_APP_SAA_ATV_DNP02",
                                                                         "NGS123_APP_SAA_ATV_4D9",
                                                                         "NGS123_APP_SAA_4D9"))

microglia_NGS123$group <- factor(microglia_NGS123$group, levels = c("NGS123_WT_ATV_DNP02",
                                                                    "NGS123_WT_ATV_4D9",
                                                                    "NGS123_APP_SAA_ATV_DNP02",
                                                                    "NGS123_APP_SAA_4D9",
                                                                    "NGS123_APP_SAA_ATV_4D9"))
...
```{r}
## Now I'll do the same for NGS000073

microglia_NGS73$group <- plyr::mapvalues(microglia_NGS73$group, from = c("ATV:RSV",
                                                                         "days_01",
                                                                         "days_07",
                                                                         "days_14",
                                                                         "days_28"),
                                                                to = c("NGS73_WT_ATV_RSV",
                                                                       "NGS73_WT_ATV_4D9_day1",
                                                                       "NGS73_WT_ATV_4D9_day7",
                                                                       "NGS73_WT_ATV_4D9_day14",
                                                                       "NGS73_WT_ATV_4D9_day28"))

microglia_NGS73$mouse <- paste0(microglia_NGS73$group, "_",microglia_NGS73$sample_id)

```

```{r}
# renaming clusters to match the manuscript names
microglia_NGS73$named_clusters <- plyr::mapvalues(Idents(microglia_NGS73), 
...
clusters <- unique(microglia_NGS73$named_clusters)
...
```{r}
p1 <- DimPlot(microglia_NGS73, group.by = "named_clusters", cols = out) + NoAxes() + ggtitle("Figure1 UMAP")
```

```{r, fig.width=18, fig.height=4}
DimPlot(microglia_NGS73, group.by = "named_clusters", cols = out, split.by = "group") + NoAxes() + ggtitle("Figure1 UMAP")
```

```{r}
p2 <- DimPlot(microglia_NGS123, 
...
```{r}
microglia_NGS123$named_clusters <- plyr::mapvalues(microglia_NGS123$seurat_clusters,
...
microglia_NGS123$named_clusters <- factor(microglia_NGS123$named_clusters,
...
names(cols_123) <- levels(microglia_NGS123$named_clusters)
```

```{r}
p2n <- DimPlot(microglia_NGS123, 
...
```{r, include=FALSE}
totals_73 <- as.data.frame((table(microglia_NGS73$named_clusters, microglia_NGS73$sample_id))) %>%
  group_by(Var2) %>%
...
```{r, include=FALSE}
totals_123 <- as.data.frame((table(microglia_NGS123$named_clusters, microglia_NGS123$sample))) %>%
  group_by(Var2) %>%
  mutate(percent = Freq/sum(Freq) *100)
...
```{r}
microglia_NGS123$broad_clusters <- plyr::mapvalues(microglia_NGS123$named_clusters, 
                                                   from = c("Homeostatic1",
                                                      "Homeostatic2",
                                                      "AbActivated1",
                                                      "AbActivated2",
...
```{r}
DimPlot(microglia_NGS123, group.by = "broad_clusters")
```

```{r}
sce <- as.SingleCellExperiment(microglia_NGS123)
